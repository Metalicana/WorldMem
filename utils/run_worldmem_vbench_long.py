"""Run VBench-Long with correct grouping for staged WorldMem videos."""
...
def reorganize_clips_results(detailed_results, dimension=None):
    ...
    videos = [
        {"video_path": path, "video_results": sum(scores) / len(scores)}
...
def main():
    ...
    original_hash = hashlib.sha256(
        inspect.getsource(utils.reorganize_clips_results).encode()
    ).hexdigest()
    print(
        "WorldMem VBench-Long grouping adapter enabled; "
        f"upstream function SHA256: {original_hash}",
        flush=True,
    )
